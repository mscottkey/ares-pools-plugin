...
export default Component.extend({
  ...
  spendPool() {
    ...
    api.requestOne('spendPool', { 
      scene_id: this.get('scene.id'),
      reason: poolSpendReason, 
      amount: poolSpendAmount, 
  ...
  addPool() {
    ...
    api.requestOne('addPool', { 
      scene_id: this.get('scene.id'),
      reason: poolAddReason, 
      amount: poolAddAmount, 
      sender: this.get('scene.poseChar.name') 
    }, null)
    .then( (response) => {
      if (response.error) {
  ...
  setSelectSpendPool(value) {
    this.set('selectSpendPool', value);
  },

  @action
  setSelectAddPool(value) {
    this.set('selectAddPool', value);
  }
});